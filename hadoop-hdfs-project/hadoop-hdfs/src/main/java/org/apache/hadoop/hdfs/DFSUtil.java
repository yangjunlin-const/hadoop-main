...
public class DFSUtil {
    ...
    /**
     * Compartor for sorting DataNodeInfo[] based on decommissioned states. Decommissioned nodes are moved to the end of
     * the array on sorting with this compartor.
     */
    ...
    /**
     * Comparator for sorting DataNodeInfo[] based on decommissioned/stale states. Decommissioned/stale nodes are moved
     * to the end of the array on sorting with this comparator.
     */
    @InterfaceAudience.Private
    public static class DecomStaleComparator implements Comparator<DatanodeInfo> {
         ...
         *
         * @param interval The time interval for marking datanodes as stale is passed from outside, since the interval
    ...
    /**
     * Whether the pathname is valid.  Currently prohibits relative paths, names which contain a ":" or "//", or other
     * non-canonical paths.
     */
    ...
    /**
     * Checks if a string is a valid path component. For instance, components cannot contain a ":" or "/", and cannot be
     * equal to a reserved component like ".snapshot".
     * <p>
     * The primary use of this method is for validating paths when loading the FSImage. During normal NN operation,
     * paths are sometimes allowed to contain reserved components.
     *
     ...
     * <p>
     * <p>
     * Note that some components are only reserved under certain directories, e.g. "/.reserved" is reserved, while
     * "/hadoop/.reserved" is not.
     *
    ...
    /**
     * @return <code>coll</code> if it is non-null and non-empty. Otherwise, returns a list with a single null value.
     */
    ...
    /**
     * Namenode HighAvailability related configuration. Returns collection of namenode Ids from the configuration. One
     * logical id for each namenode in the in the HA setup.
     *
     * @param conf configuration
    ...
    /**
     * Given a list of keys in the order of preference, returns a value for the key in the given order from the
     * configuration.
     *
     ...
     * @param nsIds
     * @param defaultAddress default address to return in case key is not found.
     * @param keys           Set of keys to look for in the order of preference   @return a map(nameserviceId to
     *                       map(namenodeId to InetSocketAddress))
     */
    private static Map<String, Map<String, InetSocketAddress>>
    ...
    /**
     * Returns list of InetSocketAddresses of corresponding to secondary namenode http addresses from the
     * configuration.
     *
     * @param conf configuration
     *
     ...
     * <p>
     * Returns namenode address specifically configured for datanodes (using service ports), if found. If not, regular
     * RPC address configured for other clients is returned.
     *
     * @param conf configuration
     *
    ...
    /**
     * Returns list of InetSocketAddresses corresponding to the namenode that manages this cluster. Note this is to be
     * used by datanodes to get the list of namenode addresses to talk to.
     * <p>
     * Returns namenode address specifically configured for datanodes (using service ports), if found. If not, regular
     * RPC address configured for other clients is returned.
     *
    ...
    /**
     * Flatten the given map, as returned by other functions in this class, into a flat list of {@link
     * ConfiguredNNAddress} instances.
     */
     ...
     * The format of this string should not be considered an interface, and is liable to change.
     */
    ...
    /**
     * Get a URI for each configured nameservice. If a nameservice is HA-enabled, then the logical URI of the
     * nameservice is returned. If the nameservice is not HA-enabled, then a URI corresponding to an RPC address of the
     * single NN for that nameservice is returned, preferring the service RPC address over the client RPC address.
     *
     * @param conf configuration
     *
     * @return a collection of all configured NN URIs, preferring service addresses
     */
    ...
    /**
     * Get a URI for each configured nameservice. If a nameservice is HA-enabled, then the logical URI of the
     * nameservice is returned. If the nameservice is not HA-enabled, then a URI corresponding to the address of the
     * single NN for that nameservice is returned.
     *
     * @param conf configuration
     * @param keys configuration keys to try in order to get the URI for non-HA nameservices
     *
    ...
    /**
     * Given the InetSocketAddress this method returns the nameservice Id corresponding to the key with matching
     * address, by doing a reverse lookup on the list of nameservices until it finds a match.
     * <p>
     * Since the process of resolving URIs to Addresses is slightly expensive, this utility method should not be used in
     * performance-critical routines.
     *
     * @param conf    - configuration
     * @param address - InetSocketAddress for configured communication with NN. Configured addresses are typically given
     *                as URIs, but we may have to compare against a URI typed in by a human, or the server name may be
     *                aliased, so we compare unambiguous InetSocketAddresses instead of just comparing URI substrings.
     * @param keys    - list of configured communication parameters that should be checked for matches.  For example, to
     *                compare against RPC addresses, provide the list DFS_NAMENODE_SERVICE_RPC_ADDRESS_KEY,
     *                DFS_NAMENODE_RPC_ADDRESS_KEY.  Use the generic parameter keys, not the NameServiceId-suffixed
     *                keys.
     *
    ...
    /**
     * Lookup the HTTP / HTTPS address of the namenode, and replace its hostname with defaultHost when it found out that
     * the address is a wildcard / local address.
     *
     ...
     * connects to the server via HTTPS if the policy is set to HTTPS_ONLY.
     *
    ...
    /**
     * Substitute a default host in the case that an address has been configured with a wildcard. This is used, for
     * example, when determining the HTTP address of the NN -- if it's configured to bind to 0.0.0.0, we want to
     * substitute the hostname from the filesystem URI rather than trying to connect to 0.0.0.0.
     *
     * @param configuredAddress the address found in the configuration
     * @param defaultHost       the host to substitute with, if configuredAddress is a local/wildcard address.
     *
    ...
    /**
     * Sets the node specific setting into generic configuration key. Looks up value of "key.nameserviceId.namenodeId"
     * and if found sets that value into generic key in the conf. If this is not found, falls back to
     * "key.nameserviceId" and then the unmodified key.
     * <p>
     * Note that this only modifies the runtime conf.
     *
     * @param conf          Configuration object to lookup specific key and to set the value to the key passed. Note the
     *                      conf object is modified.
     * @param nameserviceId nameservice Id to construct the node specific key. Pass null if federation is not
     *                      configuration.
     * @param nnId          namenode Id to construct the node specific key. Pass null if HA is not configured.
    ...
    /**
     * Get nameservice Id for the {@link NameNode} based on namenode RPC address matching the local node address.
     */
    ...
    /**
     * Get the nameservice Id by matching the {@code addressKey} with the the address of the local node.
     * <p>
     * If {@link DFSConfigKeys#DFS_NAMESERVICE_ID} is not specifically configured, and more than one nameservice Id is
     * configured, this method determines the nameservice Id by matching the local node's address with the configured
     * addresses. When a match is found, it returns the nameservice Id from the corresponding configuration key.
     *
     * @param conf       Configuration
    ...
    /**
     * Returns nameservice Id and namenode Id when the local host matches the configuration parameter {@code
     * addressKey}.<nameservice Id>.<namenode Id>
     *
     * @param conf       Configuration
     ...
     *
     * @return Array with nameservice Id and namenode Id on success. First element in the array is nameservice Id and
     * second element is namenode Id. Null value indicates that the configuration does not have the the Id.
     *
     * @throws HadoopIllegalArgumentException on error
     */
    ...
    /**
     * For given set of {@code keys} adds nameservice Id and or namenode Id and returns {nameserviceId, namenodeId} when
    ...
    /**
     * Map a logical namenode ID to its service address. Use the given nameservice if specified, or the configured one
     * if none is given.
     *
     * @param conf Configuration
    ...
    /**
     * If the configuration refers to only a single nameservice, return the name of that nameservice. If it refers to 0
     * or more than 1, return null.
     */
     ...
     *
     * @return DFS_WEB_AUTHENTICATION_KERBEROS_KEYTAB_KEY if the key is not empty else return defaultKey
     */
    ...
    /**
     * Get http policy. Http Policy is chosen as follows: <ol> <li>If hadoop.ssl.enabled is set, http endpoints are not
     * started. Only https endpoints are started on configured https ports</li> <li>This configuration is overridden by
     * dfs.https.enable configuration, if it is set to true. In that case, both http and https endpoints are
     * stared.</li> <li>All the above configurations are overridden by dfs.http.policy configuration. With this
     * configuration you can set http-only, https-only and http-and-https endpoints.</li> </ol> See hdfs-default.xml
     * documentation for more details on each of the above configuration settings.
     */
    ...
    /**
     * Return a HttpServer.Builder that the journalnode / namenode / secondary namenode can use to initialize their HTTP
     * / HTTPS server.
     */
    ...
    /**
     * Leverages the Configuration.getPassword method to attempt to get passwords from the CredentialProvider API before
     * falling back to clear text in config - if falling back is allowed.
     *
    ...
    /**
     * Assert that all objects in the collection are equal. Returns silently if so, throws an AssertionError if any
     * object is not equal. All null values are considered equal.
     *
     ...
     *
     * @throws IOException if the KeyProvider is improperly specified in the Configuration
     */
    ...
    /**
     * Probe for HDFS Encryption being enabled; this uses the value of the option {@link
     * DFSConfigKeys.DFS_ENCRYPTION_KEY_PROVIDER_URI}, returning true if that property contains a non-empty,
